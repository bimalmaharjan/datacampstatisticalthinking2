...
def get_beak_data():
	...
	df_beak = pd.DataFrame(beak_depth,columns=['beak_depth'])
	df_year = pd.DataFrame(year, columns=['year'])
	df = df_beak.join(df_year)
	return df
...
def get_beak_length():
	...
	return bl_1975,bl_2012

def plot_ecdf(df):

	bd_1975,bd_2012 = get_beak_depth(df)
...
def get_beak_depth(df):

	bd_1975 = np.array(df['beak_depth'][df['year']==1975])
	bd_2012 = np.array(df['beak_depth'][df['year']==2012])

	return bd_1975,bd_2012



def perform_hypothesis_test(bd_1975,bd_2012):
	...
	mean_diff = get_mean_difference(bd_1975,bd_2012)
...
def get_mean_difference(bd_1975,bd_2012):
	mean_diff = np.mean(bd_2012) - np.mean(bd_1975)
	return mean_diff



def plot_scatter_plot(bd_1975,bd_2012,bl_1975,bl_2012):
...
def get_confidence_intervals(bd_1975,bd_2012):
...
def compute_linear_regression(bd_1975,bd_2012,bl_1975,bl_2012):
	...
	display_linear_regression_results(bd_1975,bd_2012,
									bl_1975,bl_2012,
									bs_slope_reps_1975, bs_intercept_reps_1975,
									bs_slope_reps_2012, bs_intercept_reps_2012)

def display_linear_regression_results(bd_1975,bd_2012,
									bl_1975,bl_2012,
									bs_slope_reps_1975, bs_intercept_reps_1975,
									bs_slope_reps_2012, bs_intercept_reps_2012):	
...
def compute_beak_to_length_ratio(bd_1975,bd_2012,bl_1975,bl_2012):
...
def eda(df):
	...
	get_confidence_intervals(bd_1975,bd_2012)
	perform_hypothesis_test(bd_1975,bd_2012)

	# get beak length
	bl_1975,bl_2012 = get_beak_length()

	# scatter plot

	plot_scatter_plot(bd_1975,bd_2012,bl_1975,bl_2012) 

	compute_linear_regression(bd_1975,bd_2012,bl_1975,bl_2012)

	compute_beak_to_length_ratio(bd_1975,bd_2012,bl_1975,bl_2012)

def get_fortis_scandens_data():
	...
	return (bd_parent_fortis, bd_offspring_fortis, bd_parent_scandens, bd_offspring_scandens)

def plot_heritablity_scatter():

	bd_parent_fortis, bd_offspring_fortis, bd_parent_scandens, bd_offspring_scandens = get_fortis_scandens_data()
		# Make scatter plots
...
def get_pearson_efficient_offspring_parental():
...
def eda_heritability():
	plot_heritablity_scatter()
	get_pearson_efficient_offspring_parental()
